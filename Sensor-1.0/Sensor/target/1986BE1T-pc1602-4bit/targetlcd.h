 ...
 *---------------------------------------------------------------------------
 *
 *  File targetlcd.h: LCD PC1602x. 4-bit interface [DB7..DB4].
...
//Select 4-bit interface [DB7..DB4]
...
#define LCD_DATA1       (LCD_BASEADDR|PN26)
...
#define LCD_RES_Act()         //nothing for PC2004
#define LCD_RES_Off()         //nothing for PC2004
...
{
    InitializePORTEx(PORT_A, 0xF0, 0, PN_BASIC | LCD_PORT_OPTS_DATA);
...
//[A0..A7] -> [DB0..DB7]
#define LCD_DB_Out()            PA_Out(0xF0)
#define LCD_DB_In()             PA_In(0xF0)
#define LCD_DB_Write(val, tmp)  PA_Write( (PA_Read() & ~0xF0) | (val & 0xF0) )
#define LCD_DB_Read(val)        val = PA_Read() & 0xF0
...
__STATIC_INLINE void InitializeLCDPorts()
{
    InitializePORTEx(PORT_A, 0xF0,    0, PN_PORTIO | LCD_PORT_OPTS_DATA);
